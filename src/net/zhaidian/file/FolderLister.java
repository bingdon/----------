...
public class FolderLister implements Ilister {
	...
	public List<?> getList(FileBean fileBean) {
		...
		//获取数据
		Cursor cursor = fileBean.getActivity().managedQuery(uri, null, fileBean.getSelection(), fileBean.getSelectionArgs(), fileBean.getOrderBy());
Utils.log("LUXS:文件夹Cursor:" + cursor);
		...
		for(int i = 0; cursor != null &&  i < cursor.getCount(); i++){  
        	...
        	if(G.SD_ROOT_PATH.equals(folderPath)){
	            ...
	            tempBean.setFileName(cursor.getString(cursor.getColumnIndexOrThrow(MediaStore.Images.Media.DISPLAY_NAME)));
	            tempBean.setFileType(fileBean.getFileType());
	            if(hasFileData(fileBean.getContext(),path)) tempBean.setFileStatus(G.COMMON);
	            else tempBean.setFileStatus(G.NOT_COMMON);
	            tempList.add(tempBean);
        	}else if(!hasData(tempList, folderPath)) {
        		try{
        			String selection = "_data LIKE ?" ;
        			String[] selectionArgs = new String[]{folderPath + "%"} ;
        			Cursor countCursor = fileBean.getActivity().managedQuery(uri, null, selection, selectionArgs, fileBean.getOrderBy());
        			String name = folderPath.substring(folderPath.lastIndexOf(File.separator) + 1);
		        	tempBean.setFileThumb(path);
		            tempBean.setFilePath(folderPath);
		            tempBean.setFileName(name +" ("+ countCursor.getCount() +")");
	...
	private boolean hasData(List<FileBean> list,String path){
		for(int i = 0; list != null &&  i < list.size(); i++){ 
			if(list.get(i).getFilePath().equals(path)) return true;
		}
		return false;
	}
